...
function buildCritiqueTurnPrompt(workerName, config, prevReports) {
    ...
    for (const [name, data] of Object.entries(prevReports)) {
        if (name !== workerName && data && !data.failed) {
            ...
            otherReportsText += (data.report || '').substring(0, 50000);
            otherReportsText += '\n';
        }
    }
    ...
    const ownText = ownReport && !ownReport.failed
        ? (ownReport.report || '').substring(0, 50000)
        : '[No previous report]';

    const focusReminder = `\nREMINDER: The original task was:\n${config.task}\nStay focused on THIS task. Do not drift into unrelated analysis.\n`;

    return `TURN ${config.turn}: CROSS-CRITIQUE RE-INVESTIGATION
${focusReminder}
You are ${workerName.toUpperCase()}. This is turn ${config.turn} of a multi-AI convergence process.
Your goal: produce a MORE COMPREHENSIVE report than your previous turn by learning from the other AI's findings.
...
=== THREE-PHASE CROSS-CRITIQUE PROTOCOL ===

PHASE 1 — RECONNAISSANCE (Gap Analysis):
Read the other AI's report carefully and compare with yours.
Identify:
- Findings the other AI reported that YOU MISSED — list each one
- File paths or code areas that you did NOT examine but the other AI did
- Categories (security/architecture/performance/quality) where you have fewer findings
- Any findings the other AI has that seem wrong or exaggerated

PHASE 2 — INVESTIGATION (Deep Re-analysis):
For EACH gap identified in Phase 1:
- Read the actual source files yourself and verify independently
- If the other AI's finding is valid, gather YOUR OWN evidence (file:line reference)
- Search for ADDITIONAL issues in the same files/areas that NEITHER AI found yet
- Re-examine areas you analyzed before — did you miss anything?

For findings you CHALLENGE:
- Provide counter-evidence with file:line references
- Explain specifically why the finding is wrong or overstated

PHASE 3 — COMPREHENSIVE REPORT:
Write your NEW report. It MUST include ALL of these:
1. RETAINED findings from your previous report (with evidence)
2. ADOPTED findings from the other AI that you independently verified (with YOUR evidence)
3. NEW findings you discovered during re-investigation
4. CHALLENGED findings with counter-evidence

CRITICAL RULES:
- Your report MUST be MORE COMPREHENSIVE than your previous turn
- Finding count should INCREASE or stay the same, never decrease (unless you withdraw with evidence)
- Do NOT just say "I agree" — for adopted findings, cite YOUR OWN file:line evidence
- Do NOT blindly accept — verify each finding by reading the actual code
- If you verified a finding, ADD it to your findings list in the JSON below

=== OUTPUT FORMAT (MANDATORY) ===

SECTION 1: DETAILED ANALYSIS
Write your full analysis in English. For each finding, include file:line evidence.

SECTION 2: STRUCTURED SUMMARY (MANDATORY - at the very end)
Your updated JSON block MUST include ALL findings (retained + adopted + new):
...
TITLE RULES: Keep titles SHORT (3-8 words), use KEY TECHNICAL TERMS.
Use SAME title wording as previous turn for unchanged findings.
For adopted findings from other AI, use THEIR title wording for consistency.
