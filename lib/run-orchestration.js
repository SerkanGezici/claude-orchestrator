...
function buildCritiqueTurnPrompt(workerName, config, prevReports) {
    ...
    const focusReminder = (taskType === 'brainstorm' || taskType === 'research')
        ? `\nREMINDER: The original question was:\n${config.task}\nStay focused on THIS question. Do not drift into general code audit.\n`
        : '';

    return `TURN ${config.turn}: CROSS-CRITIQUE & IMPROVEMENT
${focusReminder}
You are ${workerName.toUpperCase()}. Critically compare and improve your analysis.
...
- YOUR FINDINGS: Still valid? Evidence strong enough?
- OTHERS' FINDINGS: What did they find that you missed? Do you agree with their evidence?
- CONSENSUS: Which findings appear in 2+ reports? (mark as high confidence)
- CONFLICTS: Where do you disagree? Who has better evidence?

PRODUCE IMPROVED REPORT with the SAME TWO-SECTION FORMAT:

SECTION 1: DETAILED ANALYSIS
- Keep valid findings with strong evidence
- Remove wrong/weak/unverified findings
- Add findings you missed from other reports (verify them yourself first)
- Note conflicts and explain your reasoning with evidence
            ...
            (async () => {
                ...
                for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
                    ...
                    try {
                        ...
                        // Output invalid — retry if attempts remain
                        ...
                        if (attempt === MAX_RETRIES) {
                            ...
                            results[name] = {
                                ...
                                failed: false,
                                attempts: attempt,
                                warning: `Output quality low: ${validation.reason}`
                            };
