...
class GeminiWrapper {
    ...
    async _executeRaw(prompt, options = {}) {
        ...
        try {
            ...
            const listResult = spawnSync('gemini', ['--list-sessions'], { cwd: targetCwd, shell: true, timeout: 10000 });
            const sessionList = (listResult.stdout || '').toString();
            const sessionMatch = sessionList.match(/(\d+)\./);
            if (sessionMatch) {
                spawnSync('gemini', ['--delete-session', sessionMatch[1]], { cwd: targetCwd, shell: true, timeout: 10000 });
