...
class GeminiWrapper {
    ...
    async _executeRaw(prompt, options = {}) {
        ...
        return new Promise((resolve, reject) => {
            ...
            // Eski session'ları sil — Gemini'nin yanlış context yüklemesini önle
            try {
                const listProc = spawn('gemini', ['--list-sessions'], { cwd: targetCwd, shell: true, stdio: ['pipe', 'pipe', 'pipe'] });
                let sessionList = '';
                listProc.stdout.on('data', d => sessionList += d);
                await new Promise(r => listProc.on('exit', r));
                // İlk session'ı sil (varsa)
                const sessionMatch = sessionList.match(/(\d+)\./);
                if (sessionMatch) {
                    const delProc = spawn('gemini', ['--delete-session', sessionMatch[1]], { cwd: targetCwd, shell: true, stdio: ['pipe', 'pipe', 'pipe'] });
                    await new Promise(r => delProc.on('exit', r));
                    logger.info(`Gemini session ${sessionMatch[1]} deleted for clean start`);
                }
            } catch (e) { /* session cleanup optional */ }
            ...
            const workspaceEnforcement = `[SYSTEM CONTEXT] Your current working directory is: ${targetCwd}\nYou MUST ONLY analyze files in this directory. This is a FRESH analysis — ignore any previous session context.\n\n`;
